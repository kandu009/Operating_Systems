 ...
 */
int IndexNode::getBlockAddress(fstream& fs, int block)
{
	if(block >= 0 && block < MAX_DIRECT_BLOCKS)
	{
		...
		readFile(fs,bytes,dataBlockOffset+indirectBlock);
		int b3 = bytes[(block-MAX_DIRECT_BLOCKS)*4+3] & 0xff ;
		int b2 = bytes[(block-MAX_DIRECT_BLOCKS)*4+2] & 0xff ;
		int b1 = bytes[(block-MAX_DIRECT_BLOCKS)*4+1] & 0xff ;
		int b0 = bytes[(block-MAX_DIRECT_BLOCKS)*4] & 0xff ;
		int dBlock = b3 << 24 | b2 << 16 | b1 << 8 | b0 ; 
		delete [] bytes;
		return dBlock;
	}
	else
 ...
 * @exception java.lang.Exception if the block number is invalid
 */
void IndexNode::setBlockAddress(fstream& fs, int block , int address)
{
	if(block >= 0 && block < MAX_DIRECT_BLOCKS)
	{
		...
		char* bytes = new char[BLOCK_SIZE];
		readFile(fs,bytes,dataBlockOffset+indirectBlock);
		bytes[(block-MAX_DIRECT_BLOCKS)*4+3] = (unsigned char) address >> 24;
		bytes[(block-MAX_DIRECT_BLOCKS)*4+2] = (unsigned char) address >> 16;
		bytes[(block-MAX_DIRECT_BLOCKS)*4+1] = (unsigned char) address >> 8;
		bytes[(block-MAX_DIRECT_BLOCKS)*4] = (unsigned char) address;
		writeFile(fs,bytes,dataBlockOffset+indirectBlock);
		delete [] bytes;
	}
	else
	{
...
void IndexNode::write(char * buffer, int offset)
{
	...
	buffer[offset+11+3*MAX_DIRECT_BLOCKS+1] = (unsigned char)(indirectBlock >> 24);
	buffer[offset+11+3*MAX_DIRECT_BLOCKS+2] = (unsigned char)(indirectBlock >> 16);
	buffer[offset+11+3*MAX_DIRECT_BLOCKS+3] = (unsigned char)(indirectBlock >> 8);
	buffer[offset+11+3*MAX_DIRECT_BLOCKS+4] = (unsigned)(indirectBlock);
...
void IndexNode::read(char * buffer , int offset)
{
	...
	b3 = buffer[offset+11+3*MAX_DIRECT_BLOCKS+1] & 0xff;
	b2 = buffer[offset+11+3*MAX_DIRECT_BLOCKS+2] & 0xff;
	b1 = buffer[offset+11+3*MAX_DIRECT_BLOCKS+3] & 0xff;
	b0 = buffer[offset+11+3*MAX_DIRECT_BLOCKS+4] & 0xff;
...
void IndexNode::setIndirectBlock(int indBlock){
...
void IndexNode::readFile(fstream& f_stream, char * bytes, int blockNumber)
{
	f_stream.seekg(blockNumber * BLOCK_SIZE);
	f_stream.read(bytes, BLOCK_SIZE);
}

void IndexNode::writeFile(fstream& f_stream, char * bytes, int blockNumber)
{
	f_stream.seekp(blockNumber * BLOCK_SIZE);
	f_stream.write(bytes, BLOCK_SIZE);
}

void IndexNode::setDataBlockOffset(int dbOffset){
	IndexNode::dataBlockOffset = dbOffset;
}
