...
bool* Kernel::unlink_marker;
char ** Kernel::pathnames;
...
int Kernel::close(int fd)
{
	...
	for( int i = 0 ; i < MAX_OPEN_FILES ; i ++ )
	{
		if(openFiles[i] == process.openFiles[fd])
		{
			delete openFiles[i];
			openFiles[i] = NULL;
			if(unlink_marker[i] == true){
				deleteFile(pathnames[i]);
			}
			break ;
		}
	}
...
//			cout <<currIndexNode.toString() << endl;
	...
	if ( indexNodeNumber < 0 )
	{
		...
		currIndexNode.setDataBlockOffset(fileSystem->getDataBlockOffset());
		int indirectBlock = openFileSystems->allocateBlock();
		currIndexNode.setIndirectBlock(indirectBlock);
		char* bytes = new char[fileSystem->getBlockSize()];
		for( int i = 0 ; i < fileSystem->getBlockSize() ;i=i+4 )
		{
			bytes[i+3] = 0x00;
			bytes[i+2] = 0xff;
			bytes[i+1] = 0xff;
		...
		fileSystem->write(bytes, fileSystem->getDataBlockOffset() + indirectBlock);
		delete[] bytes;
...
//			cout << "Next : " << nextDirectoryEntry.toString() << endl;
	...
	else
	{
		...
		{
			int address = currIndexNode.getBlockAddress(fileSystem->file,i) ;
			if( address != FileSystem::NOT_A_BLOCK )
			{
				fileSystem->freeBlock(address);
				currIndexNode.setBlockAddress(fileSystem->file, i , FileSystem::NOT_A_BLOCK);
			}
		}
		...
		fileDescriptor = new FileDescriptor(fileSystem , currIndexNode, flags);
		// assign inode for the new filecr
...
//
	...
	unlink_marker = new bool[MAX_OPEN_FILES];
	pathnames = new char*[MAX_OPEN_FILES];

	for(int i=0;i<MAX_OPEN_FILES;i++)
	{
		pathnames[i] = new char[1024];
		memset(pathnames[i], '\0', 1024);
		unlink_marker[i] = false;
		openFiles[i] = NULL;
	}
	...
	IndexNode::setDataBlockOffset(openFileSystems->getDataBlockOffset());
...
int Kernel::link(char* pathname1,char* pathname2){
	int status = 0;
	char * fullPath1 = getFullPath(pathname1);
	IndexNode inode;
	short indexNodeNumber = findIndexNode(fullPath1,inode);
	if( indexNodeNumber < 0 )
	{
		return -1 ;
	}
	char * fullPath2 = getFullPath(pathname2);
	char dirname[1024];
	memset(dirname, '\0', 1024);
	strcpy(dirname,"");
	...
	short indexNodeNumber2 = FileSystem::ROOT_INDEX_NODE_NUMBER ;
	char * token = NULL;
	token = strtok(fullPath2, "/");
	char name[512];// = "." ; // start at root node
	memset(name, '\0', 512);
	strcpy(name, ".");	//may be not needed. 
	...
	while(1)
	{
		if(token != NULL)
		{
			...
			currIndexNode.copy(prevIndexNode);
			emptyIndexNode.copy(currIndexNode);
			indexNodeNumber2 = findNextIndexNode(openFileSystems, prevIndexNode, name, currIndexNode);
	...
	if ( indexNodeNumber2 < 0 )
	{
		...
		DirectoryEntry newDirectoryEntry(indexNodeNumber , name);
		fileSystem->writeIndexNode(&inode, indexNodeNumber);
		DirectoryEntry currentDirectoryEntry;
		int read_status;
		while(true){
			read_status = readdir(dir, currentDirectoryEntry);
	...
	fileSystem->readIndexNode(&inode, indexNodeNumber);
	return status;
}
...
int Kernel::unlink(char* pathname){
	...
	short nlinks = (currIndexNode.getNlink())-1;
	if(nlinks<=0){
		FileDescriptor * fileDescriptor = new FileDescriptor( 
				openFileSystems, currIndexNode , O_RDWR ) ;
		fileDescriptor->setIndexNodeNumber( indexNodeNumber ) ;
		for(int i=0; i<MAX_OPEN_FILES; i++){
			if(openFiles[i] == fileDescriptor){
				unlink_marker[i] = true;
		...
		currIndexNode.setNlink(nlinks);
		fileSystem->writeIndexNode(&currIndexNode, indexNodeNumber);
		removeDirectoryEntry(name,dirname);
	}

	
}

int Kernel::deleteFile(char* path){
...
//			cout <<currIndexNode.toString() << endl;
	...
	else
	{
		...
		for( int i = 0 ; i < blocks ; i ++ )
		{
			int address = currIndexNode.getBlockAddress(openFileSystems->file,i) ;
			if( address != FileSystem::NOT_A_BLOCK )
			{
				fileSystem->freeBlock(address);
				currIndexNode.setBlockAddress(openFileSystems->file, i , FileSystem::NOT_A_BLOCK);
			}
		}
		int indir_Block = currIndexNode.getIndirectBlock();
		fileSystem->freeBlock(indir_Block);
		currIndexNode.setBlockAddress(openFileSystems->file, indir_Block , FileSystem::NOT_A_BLOCK);
		currIndexNode.setIndirectBlock(FileSystem::NOT_A_BLOCK);
...
int Kernel::removeDirectoryEntry(char* name, char* dirname){
...
//			cout << "here!!!" << endl;
		...
		//cout << "Entry Name : " <<  directoryEntry.getName() << endl;
		if(strcmp(directoryEntry.getName(), name) == 0)
		{
			while(readdir(dir, nextDirectoryEntry)){
				/*
				DirectoryEntry nullify(NULL, NULL);
				nullify.write( file->getBytes() , file->getOffset() % blockSize ) ;
				*/
				...
				status = writedir(dir, nextDirectoryEntry);
				if( status < 0 )
				{
					return status ;
				}
